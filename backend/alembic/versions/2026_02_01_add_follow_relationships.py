...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("pending", "accepted", "rejected", name="followstatus"),
            nullable=False,
        ),
        sa.Column(
            "created_at",
    ...
    op.create_index(
        "ix_follow_relationships_follower_id", "follow_relationships", ["follower_id"]
    )
    op.create_index(
        "ix_follow_relationships_following_id", "follow_relationships", ["following_id"]
    )
    op.create_index(
        "ix_follow_relationships_status", "follow_relationships", ["status"]
    )
    ...
    op.add_column(
        "user_preferences",
        sa.Column(
            "is_discoverable", sa.Boolean(), nullable=False, server_default="true"
        ),
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_follow_relationships_following_id", table_name="follow_relationships"
    )
    op.drop_index(
        "ix_follow_relationships_follower_id", table_name="follow_relationships"
    )
